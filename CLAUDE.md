...
### SESSION-LIFECYCLE (ABSOLUTE PFLICHT):

**Nach JEDER abgeschlossenen Aufgabe (Phase/Story/Epic) MUSST du:**

1. **Handover ins Wiki schreiben:** `wiki/discussions/YYYY-MM-DD-HHMM-session-handover.md`
   - Was wurde erledigt
   - Welche Entscheidungen getroffen
   - Was kommt als naechstes
   - Offene Fragen
2. **Alle Aenderungen committen** (git add -A + git commit)
3. **Per Telegram melden:** "Aufgabe X abgeschlossen, starte Session-Reset"
4. **Session-Reset ausfuehren:** `bash scripts/session-save-and-clear.sh "Phase X fertig"`
5. Die neue Session startet automatisch und liest Wiki + CLAUDE.md + Handover

**VERBOTEN:** Mehrere Aufgaben in einer Session abarbeiten.
**VERBOTEN:** Kontext ueber 70% laufen lassen ohne Reset.

**Wissen bleibt erhalten weil ALLES im Wiki ist (Regel 11).**
...
## 11. KNOWLEDGE WIKI (Karpathy LLM Wiki Pattern) — ABSOLUTE PFLICHT

**ALLES MUSS INS WIKI. NICHTS DARF NUR IN .planning/ LIEGEN.**

- Vor jeder Arbeit: `wiki/index.md` lesen
- Wiki-Schema: Siehe `AGENTS.md`
- **Nightly Deep Research:** 02:00-05:00 AM Berlin via Cron — Gemini durchsucht Internet, Wiki wird aktualisiert
- Wissen waechst exponentiell — kein Kontextverlust
- **Ziel:** Am Ende koennen wir alles testen und die App trainieren

### Wiki-Struktur (PFLICHT):

```
wiki/
├── index.md              → Master-Index aller Seiten
├── log.md                → Chronologie aller Aenderungen
├── code/                 → Jede Funktion/Klasse/Modul dokumentiert
├── corrections/          → Jede Korrektur (YYYY-MM-DD-topic.md)
├── discussions/          → Jede 3-KI-Diskussion (YYYY-MM-DD-topic.md)
├── research/             → Jede Recherche von Gemini
├── decisions/            → Jede Entscheidung (YYYY-MM-DD-topic.md)
├── tests/                → Jedes Test-Modul
├── bugs/                 → Jeder Bug+Fix (YYYY-MM-DD-bug.md)
└── knowledge/            → Jede neue Erkenntnis
```

### Was MUSS ins Wiki:
- Jeder Code (Funktionen, Klassen, Module) → `wiki/code/`
- Jede Korrektur → `wiki/corrections/`
- Jede Diskussion (Claude+Gemini+Codex) → `wiki/discussions/`
- Jede Recherche → `wiki/research/`
- Jede Entscheidung → `wiki/decisions/`
- Jeder Test → `wiki/tests/`
- Jeder Bug+Fix → `wiki/bugs/`
- Jede neue Erkenntnis → `wiki/knowledge/`

### Drei Schichten:
```
raw/sources/    → Unveraenderliche Quelldokumente
wiki/           → LLM-generierte Markdown-Seiten (PFLICHT befuellt)
...
### Nightly Research (Cron 02:00):
```bash
# Gemini durchsucht jede Nacht das Internet zu zufaelligen Blueprint-Topics
gemini -p "AeroCloud Engine: Was ist neu seit gestern in [topic]?"
# Ergebnisse direkt ins wiki/research/ ingestieren
# Per Telegram an Jens senden
```
